...
public class CalculationService : ICalculationService
{
    ...
    public async Task<long[]> CheckUserIds(long[] ids,
        CancellationToken cancellationToken)
    {
        using var transaction = _calculationRepository.CreateTransactionScope();
        var res = await _calculationRepository.GetUserIds(ids, cancellationToken);
        return res;
    }

    public async Task<bool> CheckCalculationsIds(long[] ids, 
        CancellationToken cancellationToken)
    {
        using var transaction = _calculationRepository.CreateTransactionScope();
        var res = await _calculationRepository.GetNotExistedIds(ids, cancellationToken);
        return res;
    }
    ...
    public async Task DeleteRequestEdentries(long userId, CancellationToken cancellationToken)
    {
        using var transaction = _calculationRepository.CreateTransactionScope();
        await _calculationRepository.DeleteRequestedEntries(userId, cancellationToken);
        return;
    }
}